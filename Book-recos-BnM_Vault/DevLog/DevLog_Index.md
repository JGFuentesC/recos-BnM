...
| [[DevLog/2026-06-11-manuel-ingest-phase2 | 2026-06-11 (sesión 16)]] | Phase 2: `buildGenreAffinity` en scoring.js, `titleLower` en ingest (tmdb + models), skip reimport (7 días), 6 tests passing, config.py fix (ruta fija a ingest/.env), checklist actualizado | Claude Code (big-pickle) + Manuel Serranía |
| [[DevLog/2026-06-12-hector-qa-assignment | 2026-06-12 (sesión 17)]] | QA Fase 2: validación de pendientes de Sprint-1 (entrega `GET /api/content/:id` confirmada completa), preparación de hoja de trabajo `Sprint-1/qa/05-Hector-QA.md` con los 39 casos asignados (Secciones 0 Pre-req, 1 Auth, 8 API) listos para ejecutar Jun 13–15. Bloqueante: URLs de Hosting/Cloud Run pendientes de Germán. | Claude Code (claude-opus-4-8) + Héctor Morales Marbán |
